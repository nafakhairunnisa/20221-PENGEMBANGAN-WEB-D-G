jQuery(document).ready(function () { jQuery(".ecs-load-more-button").each(function () { widget = jQuery(this); settings = widget.attr("data-settings"); args = JSON.parse(settings); widget.children(".elementor-button").attr("href", "javascript:ECS_load_next_page('" + args.widget_id + "');"); }); }); var canBeLoaded = true; function ECS_load_next_page(id) {
    ...
    data = { 'action': 'ecsload', 'query': ecs_ajax_params.posts, 'ecs_ajax_settings': settings, }; jQuery.ajax({
        url: ecs_ajax_params.ajaxurl, data: data, type: 'POST', beforeSend: function (xhr) { if (args.load_method == 'loadmore') button_text.html(attb.loading_text); canBeLoaded = false; }, success: function (data) {
            if (data) {
                posts.append(data); args.current_page++; if (args.load_method == 'loadmore') { button_text.html(attb.text); button_text.blur(); }
                ...
                canBeLoaded = true; if (typeof ECScheckInView !== 'undefined') ECScheckInView();
            } else {
